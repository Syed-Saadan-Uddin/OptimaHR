...
const Header: React.FC<HeaderProps> = ({ role, collapsed }) => {
        ...
        <button className="flex items-center gap-3 pl-2 pr-1 py-1 hover:bg-off-white rounded-full transition-all">
          <div className="text-right">
            <p className="text-xs font-bold text-navy">
              {role === 'CANDIDATE' ? 'Guest User' : 'John Doe'}
            </p>
            <p className="text-[10px] text-slate-500">
              {role === 'HR_ADMIN' ? 'Global Admin' : role === 'CANDIDATE' ? 'Applicant' : 'Engineering Dept'}
            </p>
          </div>
